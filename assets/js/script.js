...
87|People|Which of these famous authors is said to have drunk 50 cups per day of coffee, enough so that caffeine poisoning partially led to his death?|Balzac|Faulkner|Dostoyevsky|Ibsen|French novelist Honor de Balzac who wrote 'La Comdie Humaine' among other works apparently couldn't get enough coffee...he drank it night and day.  Well perhaps he did get enough of it in retrospect...much more than enough!  He seemingly didn't subscribe to the ancient Greek maxim 'nothing in excess'. 
...
const Game = {
  questions: [],
  backgroundMusic: '',
  generateTemplate: function(config) { //display, step, percent
    var i = Math.floor(Math.random() * (Game.questions.length - 0) + 0);
  
    var template = `<div class="multisteps_form_panel step active" style="display: ${config.display};">
       ...
       <div class="content_box py-5 ps-5 position-relative">
          ...
          <div class="step_progress_bar mb-3">
             <div class="progress rounded-pill">
                ...
                <div class="progress-bar mx-2 rounded-pill" role="progressbar" style="width: ${config.percent}%;" aria-valuenow="${config.percent}" aria-valuemin="0" aria-valuemax="100"></div>
             </div>
          </div>
          <div class="question_number text-capitalize">
             <span class="text-white">question ${config.step} / 4</span>
             <p class="pt-3">------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------</p>
          </div>
          <div class="question_title py-3">
             <h1 class="text-white">${Game.questions[i].question}</h1>
          </div>
          <div class="form_items answers_${config.step}" question_index="${parseInt(i)}">
             <label for="opt_${config.position[0]}" class="step_1 animate__animated animate__fadeInRight animate_25ms position-relative rounded-pill text-start text-white">
             ${Game.questions[i].randomAnswers[0]}
                <input id="opt_${config.position[0]}" type="radio" name="stp_${config.step}_select_option" value="${Game.questions[i].randomAnswers[0]}" checked="">
             </label>
             <label for="opt_${config.position[1]}" class="step_1 animate__animated animate__fadeInRight animate_50ms position-relative rounded-pill text-start text-white active">
             ${Game.questions[i].randomAnswers[1]}
                <input id="opt_${config.position[1]}" type="radio" name="stp_${config.step}_select_option" value="${Game.questions[i].randomAnswers[1]}">
             </label>
             <label for="opt_${config.position[2]}" class="step_1 animate__animated animate__fadeInRight animate_75ms position-relative rounded-pill text-start text-white">
             ${Game.questions[i].randomAnswers[2]}
                <input id="opt_${config.position[2]}" type="radio" name="stp_${config.step}_select_option" value="${Game.questions[i].randomAnswers[2]}">
             </label>
             <label for="opt_${config.position[3]}" class="step_1 animate__animated animate__fadeInRight animate_75ms position-relative rounded-pill text-start text-white">
             ${Game.questions[i].randomAnswers[3]}
                <input id="opt_${config.position[3]}" type="radio" name="stp_${config.step}_select_option" value="${Game.questions[i].randomAnswers[3]}">
             </label>
          </div>
       </div>
    </div>
...
function playMusic (audioName) {
  Game.backgroundMusic = document.createElement('audio');
  Game.backgroundMusic.volume = 0.3;
  Game.backgroundMusic.loop = true;
  Game.backgroundMusic.src = 'assets/audio/' + audioName + ".mp3";
  Game.backgroundMusic.addEventListener("canplaythrough", () => {
...
function parseData(inputData) {
  var triviaLines = inputData.split('\n');
  triviaLines.forEach(line => {
    ...
    triviaObject.randomAnswers = shuffle(randomAnswers);
    Game.questions.push(triviaObject);
  });

  $('#panel').empty();


  let thisTemplate = Game.generateTemplate( {display: 'block', step: 1, percent: 0, position: [1,2,3,4]});
  $('#panel').append(thisTemplate);

  thisTemplate = Game.generateTemplate( {display: 'none', step: 2, percent: 25, position: [5,6,7,8]});
  $('#panel').append(thisTemplate);

  thisTemplate = Game.generateTemplate( {display: 'none', step: 3, percent: 75, position: [9,10,11,12]});
  $('#panel').append(thisTemplate);

  thisTemplate = Game.generateTemplate( {display: 'none', step: 4, percent: 100, position: [13,14,15,16]});
  $('#panel').append(thisTemplate);
...
$( document ).ready(function() {
  ...
  Swal.fire({
    ...
    confirmButtonColor: '#3085d6',
    cancelButtonColor: '#d33',
    confirmButtonText: 'Music on',
    cancelButtonText: 'Music off',
  }).then((result) => {
    if (result.isConfirmed) {
      playMusic('music');
    } else {
